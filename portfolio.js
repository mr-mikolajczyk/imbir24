...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  //   let pheadHeight =  document.querySelector('.portfolio_heading').offsetHeight;
  //   const rootElement = document.documentElement;
  //   rootElement.style.setProperty('--pf-margin', pheadHeight + 'px');

  let pheadHeight; // Declare pheadHeight outside the functions for better performance
  const rootElement = document.documentElement;
  function setMargin() {
    pheadHeight = document.querySelector(".portfolio_heading").offsetHeight;
    rootElement.style.setProperty("--pf-margin", pheadHeight + "px");
  }
  ...
  gsap.set(".portfolio_item_wrapper", { opacity: 0, yPercent: 15, scale: 0.9 });
  ScrollTrigger.batch(".portfolio_item_wrapper", {
    start: "top 85%",
    onEnter: (batch) =>
      gsap.to(batch, {
        yPercent: 0,
